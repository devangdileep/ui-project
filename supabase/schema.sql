...
create table if not exists public.packages (
  ...
  rating numeric(2, 1) not null default 4.7 check (rating >= 1.0 and rating <= 5.0),
  image text,
  description text,
  created_at timestamptz not null default now()
);
...
create policy "read own profile or agent view all"
on public.profiles for select
using (auth.uid() = id or public.is_agent());
...
values
  ...
  (
    ...
    'Monastery tours, lake excursions, local organic foods, and organic farm stays.'
  )
